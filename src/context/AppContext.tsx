...
export const AppProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      dispatch({ type: 'SET_ERROR', payload: 'Errore nel caricamento iniziale' });
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false });
    }
  };
  ...
  useEffect(() => {
    loadInitialData(); // Carica i dati all'inizializzazione
  }, []);
  ...
  const updateTask = async (id: number, taskData: Task) => {
    dispatch({ type: 'SET_LOADING', payload: true });
    try {
      const updatedTask = await apiService.updateTask(id.toString(), taskData);
