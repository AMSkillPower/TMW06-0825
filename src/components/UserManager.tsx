...
const UserManager: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    fullName: '',
    email: '',
    role: 'User' as 'Main' | 'User',
    isActive: true
  });

  // Verifica che l'utente corrente sia Main
                ...
                <tr key={user.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex px-2 py-1 rounded-full text-xs font-semibold ${
                      ...
                      {user.role === 'Main' ? 'Amministratore' : 'Utente'}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
          ...
          <div className="bg-white rounded-lg p-4 lg:p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  value={formData.role}
                  onChange={(e) => setFormData({ ...formData, role: e.target.value as 'Main' | 'User' })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm lg:text-base"
                >
                  <option value="User">Utente</option>
                  <option value="Main">Amministratore</option>
                </select>
              </div>
